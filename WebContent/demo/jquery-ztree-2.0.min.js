 ...
 *
 */
(function(e){function Ba(b,a){a.unbind(U);a.bind(U,function(c,f,d){typeof b.callback.click=="function"&&b.callback.click(c,f,d)});a.unbind(V);a.bind(V,function(c,f,d){typeof b.callback.change=="function"&&b.callback.change(c,f,d)});a.unbind(W);a.bind(W,function(c,f,d){typeof b.callback.rename=="function"&&b.callback.rename(c,f,d)});a.unbind(X);a.bind(X,function(c,f,d){typeof b.callback.remove=="function"&&b.callback.remove(c,f,d)});a.unbind(Y);a.bind(Y,function(c,f,d){typeof b.callback.drag=="function"&&
...
f.hover(function(){if(b.dragStatus==0){I(a);J(a);ma(b,a);na(b,a)}},function(){I(a);J(a)});f.bind("mousedown",function(j){if(!(j.button==2||!b.editable)){var h=document,i,l,Ca=j.clientX,Da=j.clientY;e(h).mousemove(function(o){if(a.editNameStatus)return true;window.getSelection?window.getSelection().removeAllRanges():document.selection.empty();if(b.dragStatus==0&&Math.abs(Ca-o.clientX)<oa&&Math.abs(Da-o.clientY)<oa)return true;e("body").css("cursor","pointer");e("#"+a.tId+q);if(b.dragStatus==0&&a.isParent&&
...
c.attr("offsetLeft")-c.attr("offsetWidth")-1*d.width()-15<0){d.remove();c.prepend(f);d=e("#"+a.tId+G)}e("#"+a.tId+G).bind("click",function(){var g=true;if(typeof b.callback.beforeRemove=="function")g=b.callback.beforeRemove(b.treeObjId,a);if(g!=false){ra(b,a);e("#"+b.treeObjId).trigger(X,[b.treeObjId,a]);return false}}).bind("mousedown",function(){return true}).show()}}function v(b,a,c){if(a){a.removeClass();b=b.checkStyle+"_"+(c.checkedNew?Fa:Ga)+"_"+(c.checkedNew||b.checkStyle==E?c.check_True_Full?
sa:ta:c.check_False_Full?sa:ta);b=c.checkboxFocus?b+"_"+Ha:b;a.addClass(Ia);a.addClass(b)}}function N(b,a){if(a&&a.parentNode){for(var c=true,f=true,d=0;d<a.parentNode.nodes.length;d++){if(b.checkStyle==E&&(a.parentNode.nodes[d].checkedNew||!a.parentNode.nodes[d].check_True_Full))c=false;else if(b.checkStyle!=E&&a.parentNode.checkedNew&&(!a.parentNode.nodes[d].checkedNew||!a.parentNode.nodes[d].check_True_Full))c=false;else if(b.checkStyle!=E&&!a.parentNode.checkedNew&&(a.parentNode.nodes[d].checkedNew||
...
t);if(c==b.root)if(c.nodes.length==1)n(d,O);else{c=e("#"+c.nodes[0].tId+q);n(c,C);n(d,y)}else n(d,y);f.removeClass(D)}}function ga(b,a){if(!b||!a)return null;for(var c=0;c<b.length;c++){if(b[c].tId==a)return b[c];var f=ga(b[c].nodes,a);if(f)return f}return null}function Ja(b){if(b.curTreeNode){I(b.curTreeNode);J(b.curTreeNode);e("#"+b.curTreeNode.tId+u).removeClass(ea);e("#"+b.curTreeNode.tId+B).text(b.curTreeNode[b.nameCol])}}function Ka(b){if(b.curEditTreeNode){e("#"+b.curEditTreeNode.tId+u).removeClass(za);
e("#"+b.curEditTreeNode.tId+ia).unbind();e("#"+b.curEditTreeNode.tId+B).text(b.curEditTreeNode[b.nameCol]);b.curEditTreeNode.editNameStatus=false;b.curEditTreeNode=null}}function R(b,a){if(!(b.curTreeNode==a&&!b.editable)){Ja(b);Ka(b);if(b.editable){ma(b,a);na(b,a)}if(a.editNameStatus){e("#"+a.tId+B).html("<input type=text class='rename' id='"+a.tId+ia+"'>");var c=e("#"+a.tId+ia);c.attr("value",a[b.nameCol]);c.focus();Ea(c.get(0),a[b.nameCol].length);c.bind("blur",function(){a[b.nameCol]=this.value;
e("#"+b.treeObjId).trigger(W,[b.treeObjId,a])}).bind("click",function(){return false}).bind("dblclick",function(){return false});e("#"+a.tId+u).addClass(za);b.curEditTreeNode=a}else e("#"+a.tId+u).addClass(ea);b.curTreeNode=a}}function Aa(b,a){if(!b)return[];for(var c=[],f=0;f<b.length;f++){if(b[f].checkedNew==a)c=c.concat([b[f]]);var d=Aa(b[f].nodes,a);if(d.length>0)c=c.concat(d)}return c}function La(){return{container:null,init:function(b){this.container=b;return this},refresh:function(){var b=
this.container.attr("id");e("#"+b).empty();m[b].curTreeNode=null;aa=0;H(m[b],0,m[b].root.nodes)},setEditable:function(b){var a=this.container.attr("id");m[a].editable=b;return this.refresh()},getNodes:function(){var b=this.container.attr("id");return m[b].root.nodes},getSelectedNode:function(){var b=this.container.attr("id");return m[b].curTreeNode},getCheckedNodes:function(b){var a=this.container.attr("id");if(a){b=b!=false;return Aa(m[a].root.nodes,b)}},getNodeByTId:function(b){var a=this.container.attr("id");
...
selectNode:function(b){var a=this.container.attr("id");if(a&&b){R(m[a],b);L(m[a],b,true,false,function(){e("#"+b.tId+t).focus().blur()})}},addNodes:function(b,a){var c=this.container.attr("id");if(c&&a){b||(b=null);wa(m[c],b,a)}},updateNode:function(b){var a=this.container.attr("id");a&&b&&e("#"+b.tId+B).text(b[m[a].nameCol])},moveNode:function(b,a){var c=this.container.attr("id");if(c&&a)if(!(b&&(a.parentNode==b||e("#"+a.tId).find("#"+b.tId).length>0))){b||(b=null);qa(m[c],b,a,false)}},removeNode:function(b){var a=
this.container.attr("id");a&&b&&ra(m[a],b)}}}var U="ZTREE_CLICK",V="ZTREE_CHANGE",W="ZTREE_RENAME",X="ZTREE_REMOVE",Y="ZTREE_DRAG",M="ZTREE_DROP",Z="ZTREE_ASYNC_SUCCESS",$="ZTREE_ASYNC_ERROR",q="_switch",t="_ico",B="_span",ia="_input",K="_edit",G="_remove",r="_ul",u="_a",O="root",C="roots",P="center",y="bottom",ka="noLine",D="line",z="open",A="close",w="docu",ea="curSelectedNode",za="curSelectedNode_Edit",fa="tmpTargetTree",S="tmpTargetNode",E="radio",Ia="chk",Ga="false",Fa="true",sa="full",ta="part",
Ha="focus",ba="all",oa="5",m=[],aa=0;e.fn.zTree=function(b,a){var c={treeObjId:"",checkable:false,editable:false,edit_renameBtn:true,edit_removeBtn:true,showLine:true,curTreeNode:null,curEditTreeNode:null,dragStatus:0,dragNodeShowBefore:false,checkStyle:"checkbox",checkType:{Y:"ps",N:"ps"},checkRadioType:"level",checkRadioCheckedList:[],async:false,asyncUrl:"",asyncParam:[],asyncParamOther:[],nameCol:"name",expandSpeed:"fast",root:{isRoot:true,nodes:[]},callback:{beforeClick:null,beforeChange:null,
beforeDrag:null,beforeDrop:null,beforeRename:null,beforeRemove:null,click:null,change:null,drag:null,drop:null,rename:null,remove:null,asyncSuccess:null,asyncError:null}};b&&e.extend(c,b);c.treeObjId=this.attr("id");c.treeObj=this;c.root.tId=-1;c.root.name="ZTREE ROOT";c.root.isRoot=true;c.checkRadioCheckedList=[];aa=0;if(a)c.root.nodes=a;m[c.treeObjId]=c;e("#"+c.treeObjId).empty();if(c.root.nodes&&c.root.nodes.length>0)H(c,0,c.root.nodes);else c.async&&c.asyncUrl&&c.asyncUrl.length>0&&va(c);Ba(c,
this);return(new La).init(this)};var T=[]})(jQuery);