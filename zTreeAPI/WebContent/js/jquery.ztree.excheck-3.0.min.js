...
c.data.key.childs,h=c.data.key.checked,f=j("#"+a.tId+i.id.CHECK);if(a!=e.getRoot(c))a.nocheck!==!0?(a[h]=b,a.check_Child_State=a[d]&&a[d].length>0?b?2:0:-1):a.check_Child_State=-1,g.setChkClass(c,f,a);if(a[d]){h=0;for(f=a[d].length;h<f;h++)a[d][h]&&g.setSonNodeCheckBox(c,a[d][h],b)}}}},event:{},data:{getRadioCheckedList:function(c){for(var a=e.getRoot(c).radioCheckedList,b=0,d=a.length;b<d;b++)e.getNodeCache(c,a[b].tId)||(a.splice(b,1),b--,d--);return a},getCheckStatus:function(c,a){if(!c.check.enable||
a.nocheck)return null;var b=c.data.key.checked;return{checked:a[b],half:c.check.chkStyle==i.radio.STYLE?a.check_Child_State===2:a[b]?a.check_Child_State>-1&&a.check_Child_State<2:a.check_Child_State>0}},getTreeCheckedNodes:function(c,a,b,d){if(!a)return[];for(var h=c.data.key.childs,f=c.data.key.checked,d=!d?[]:d,k=0,g=a.length;k<g;k++)a[k].nocheck!==!0&&a[k][f]==b&&d.push(a[k]),e.getTreeCheckedNodes(c,a[k][h],b,d);return d},getTreeChangeCheckedNodes:function(c,a,b){if(!a)return[];for(var d=c.data.key.childs,
h=c.data.key.checked,b=!b?[]:b,f=0,g=a.length;f<g;f++)a[f].nocheck!==!0&&a[f][h]!=a[f].checkedOld&&b.push(a[f]),e.getTreeChangeCheckedNodes(c,a[f][d],b);return b},makeChkFlag:function(c,a){if(a){var b=c.data.key.childs,d=c.data.key.checked,h=-1;if(a[b])for(var f=0,g=a[b].length;f<g;f++){var e=a[b][f],j=-1;if(c.check.chkStyle==i.radio.STYLE)if(j=e.nocheck===!0?e.check_Child_State:e.nocheck!==!0&&e[d]?2:e.check_Child_State>0?2:0,j==2){h=2;break}else j==0&&(h=0);else if(c.check.chkStyle==i.checkbox.STYLE)if(j=
e.nocheck===!0?e.check_Child_State:e.nocheck!==!0&&e[d]?e.check_Child_State===-1||e.check_Child_State===2?2:1:e.check_Child_State>0?1:0,j===1){h=1;break}else if(j===2&&f>0&&j!==h){h=1;break}else if(h===2&&j>-1&&j<2){h=1;break}else j>-1&&(h=j)}a.check_Child_State=h}}}});var n=j.fn.zTree,m=n._z.tools,i=n.consts,g=n._z.view,e=n._z.data;e.exSetting(s);e.addInitBind(function(c){var a=c.treeObj,b=i.event;a.unbind(b.CHECK);a.bind(b.CHECK,function(a,b,f){m.apply(c.callback.onCheck,[a,b,f])})});e.addInitCache(function(){});
